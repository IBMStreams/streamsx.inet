...
InetRetriever::InetRetriever(const std::string& targURL) throw(int) :
  _targetURL(targURL)
  {
    reset();
...
InetRetriever::~InetRetriever()
  {
    curl_easy_cleanup(_curlHandle);
  }

std::string InetRetriever::targetURL() const
  {
    return _targetURL;
  }

std::string InetRetriever::fetch() throw(CURLcode)
  {
    _curlBuffer.clear();
    _dataCameBack = false;
...
std::string InetRetriever::lastknownContents() const
  {
    return _fileCache;
  }

size_t InetRetriever::bytesTransferredTotal()
  {
    return _bytesTransferredTotal;
  }

size_t InetRetriever::bytesTransferredLastFetch()
  {
    return _bytesTransferredLastFetch;
  }

void InetRetriever::enableCurlTrace()
  {
    curl_easy_setopt(_curlHandle, CURLOPT_VERBOSE, 1);
  }

void InetRetriever::disableCurlTrace()
  {
    curl_easy_setopt(_curlHandle, CURLOPT_VERBOSE, 0);
  }

void InetRetriever::incrementalFetch()
  {
    _incrementalFetch = true;
  }

void InetRetriever::incrementalFetch(bool whether)
  {
    _incrementalFetch = whether;
  }

void InetRetriever::ignoreLastModTime()
  {
    _ignoreLastModTime = true;
  }

void InetRetriever::ignoreLastModTime(bool whether)
  {
    _ignoreLastModTime = whether;
  }

CURL* InetRetriever::curlHandle()
  {
    return _curlHandle;
  }

size_t InetRetriever::consumeData_static(void* buffer, const size_t recSize,
...
size_t InetRetriever::consumeData(const void* data, const size_t recSize,
