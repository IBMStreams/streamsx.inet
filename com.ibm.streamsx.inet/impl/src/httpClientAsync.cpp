...
namespace httpClient {
	...
	struct BodyHandler {
		...
		BOOST_NETWORK_HTTP_BODY_CALLBACK(operator(), range, error) {
			...
			if (error) {
				if(error == asio::error::eof) // not a real error - the whole body received
	...
	public:
		...
		HttpClientImpl( SubmitHandler submitHandler, int timeout) : io_service_(std::make_shared<io_service>()),
